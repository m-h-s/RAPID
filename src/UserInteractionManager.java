...
public class UserInteractionManager {
	...
	public void processUserInput(String UserInput) {
		...
		/* 
		 * Right now the switch statements do not work correctly. The logic of
		 * exit is not correct.
		 */
	...
	public void exploreDesigns(String UserInput) {
		if (!Objects.equals(UserInput.toLowerCase().replaceAll("\\s", ""), "exit")) {
			...
			printInConversationHistory("Question", "Do you want to explore answers one by one or find the best answer? ");
			...
			for (int i = 0; i < Answer.size(); i++)
               ...
               /*
					...
					{ printInConversationHistory("Answer", Answer.get(i), -1);
					  System.out.println("ZZZZZZZZZZZZZZZZ"+ Answer.get(i));
					}
					*/
				else
...
/*	
	...
	*################ This version got the keywords for the requirements one by one. 
	*################ It did not take a complete sentence.
	public void processRequirementsOldVersion(String UserInput) {
		...
		if (!UserInput.replaceAll("\\s", "").toLowerCase().contains("exit")) {
			...
			if (this.ExpectedItem == ExpectedQueryItem.Req) 
			{
				...
				// UI.appendToConversationHistory ("magenta", userInputforTerm.printTerm() +"\n");
				printInConversationHistory("Other", userInputforTerm.printTerm());
				this.ExpectedItem = ExpectedQueryItem.PRIORITY;
				getRequirementsSetFromUser();
			} 
				...
				UserInputForReqTopic = UserInput;
				userInputforTerm = new Term(UserInputForReqType, UserInputForReqTopic);
				// UI.appendToConversationHistory ("magenta", userInputforTerm.printTerm()+"\n");
				printInConversationHistory("Other", userInputforTerm.printTerm());
				this.ExpectedItem = ExpectedQueryItem.PRIORITY;
				getRequirementsSetFromUser();
				...
				this.ExpectedItem = ExpectedQueryItem.Req;
				userInputforExpectedSatsifaction = TermSatisfactionValues.valueOf(UserInput.toUpperCase());
				//userInputforExpectedSatsifaction = TermSatisfactionValues.SAT;
				this.ReqSet.addQueriedTerm(userInputforTerm, userInputForPriority, userInputforExpectedSatsifaction);
				// printInConversationHistory("Other", userInputforExpectedSatsifaction.toString());
				getRequirementsSetFromUser();
			}
		} else {
			printInConversationHistory("Other", "exit");
...
/*
	public void printInConversationHistory(String OutputType, String Output, int i) {
		if ((OutputType == "Answer" || OutputType == "Question") && i == -1) {
			...
			Timer userInputforTerm = new Timer(1000, null);
			userInputforTerm.start();
